...
public class SchemaExtractionToolIT extends BaseTest {
    ...
    public void testCreateIndexStatement() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
            ...
            String createIndexStatement1 = "CREATE INDEX "+indexName1 + " ON "+pTableFullName+"(v1 DESC) INCLUDE (v2)";
            ...
            String [] args2 = {"-tb", tableName, "-s", schemaName, "--tree-file", "/tmp/treeIndex.json"};
            set.run(args2);
            System.out.println(set.output);
        }
    }
    ...
    public void testCreateViewStatement() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
            ...
            String [] args2 = {"-tb", tableName, "-s", schemaName, "--tree-file", "/tmp/treeView.json"};
            set.run(args2);
